...
mod tests {
    ...
    /// Regression test for issue #6822: try_tool_call_parse_xml must return both tool
    /// calls when two parallel Hermes/Qwen3Coder-style calls arrive in one message.
    #[test]
    fn test_parse_parallel_tool_calls_single_chunk() {
        let config = XmlParserConfig::default();
        let input = "<tool_call>\n\
...
<parameter=timezone>Europe/Paris</parameter>\n\
        ...
        let (calls, _normal) = try_tool_call_parse_xml(input, &config, None).unwrap();
        assert_eq!(
            calls.len(),
            2,
